...
// x3 rgb fringe: r/b read the fold one tap either side, 0 = no split
...
void main() {
    ...
    vec2 px = vec2(0.0016, 0.0021) * (1.0 + u_x3 * 6.0);
    vec3 tp = texture2D(u_tex0, v_texcoord + px).rgb;
    vec3 tm = texture2D(u_tex0, v_texcoord - px).rgb;
    vec3 c = (texture2D(u_tex0, v_texcoord).rgb + tp + tm
    ...
    vec3 z = lum + (vec3(dot(tp, W), lum, dot(tm, W)) - lum) * u_x3 * 6.0;
    z = z * (1.0 + u_x2 * 3.0);
    z = abs(fract(z) * 2.0 - 1.0);               // brightness fold-over
    gl_FragColor = vec4(hsb2rgb(vec3(hue, 0.85, 1.0)) * z, 1.0);
}
